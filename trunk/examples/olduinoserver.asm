; generated by lcc-xr182b $Version: 5.0 - XR182b $ on Wed Jun 05 17:18:03 2013
...
;  	if (strindex((char *)buf,"POST /")>=0) reqtype=post;
	ldaD R12,_buf; reg:acon
	ldaD R13,L77; reg:acon
	Ccall _strindex; CALLI2(ar)
	jcI2I r15,0,lbnf,L75  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
	ldaD R11,1; reg:acon
	st2 R11,'D',(_reqtype),0; ASGNI2(addr,reg)*;
L75:
;	if (strindex((char *)buf,"GET /")>=0) reqtype=get;
	ldaD R12,_buf; reg:acon
	ldaD R13,L80; reg:acon
	Ccall _strindex; CALLI2(ar)
	jcI2I r15,0,lbnf,L78  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
	ld2z R11
	st2 R11,'D',(_reqtype),0; ASGNI2(addr,reg)*;
L78:
;	if (strindex((char *)buf,"GET /favicon")>=0) reqtype=favicon;
	ldaD R12,_buf; reg:acon
	ldaD R13,L83; reg:acon
	Ccall _strindex; CALLI2(ar)
	jcI2I r15,0,lbnf,L81  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
	ldaD R11,2; reg:acon
	st2 R11,'D',(_reqtype),0; ASGNI2(addr,reg)*;
L81:
;	printf("reqtype=%d\n",reqtype);
	ldaD R12,L84; reg:acon
	ld2 R13,'D',(_reqtype),0 ;reg:INDIRI2(addr)
	Ccall _printf
;	if (reqtype==post){
...
;	  if (strindex((char *)buf,"rd=0") > 0)
	ldaD R12,_buf; reg:acon
	ldaD R13,L90; reg:acon
	Ccall _strindex; CALLI2(ar)
	jnI2I r15,0,lbdf,L88 ;LEI2 0 15 L88; LE is flipped test & subtraction
;		ledmode=0;
	ld2z R11
	st2 R11,'D',(_ledmode),0; ASGNI2(addr,reg)*;
L88:
;	  if (strindex((char *)buf,"rd=1") > 0)
	ldaD R12,_buf; reg:acon
	ldaD R13,L93; reg:acon
	Ccall _strindex; CALLI2(ar)
	jnI2I r15,0,lbdf,L86 ;LEI2 0 15 L86; LE is flipped test & subtraction
;		ledmode=1;
	ldaD R11,1; reg:acon
	st2 R11,'D',(_ledmode),0; ASGNI2(addr,reg)*;
;	}
...
;		printf("favicon ignored\n");
	ldaD R12,L99; reg:acon
	Ccall _printf
	lbr L98
L97:
;		printf("unknown input\n");
	ldaD R12,L100; reg:acon
	Ccall _printf
L98:
L95:
L86:
...
;	switch (reqtype){
	ld2 R11,'D',(_reqtype),0 ;reg:INDIRI2(addr)
	jzU2 r11,L111; EQ 0
	jeqU2I r11,1,L111;EQI2(reg,con)
	jeqU2I r11,2,L113;EQI2(reg,con)
	lbr L109
L111:
;			printf("sending form \n");
...
;  	switch (socketstatus){
	...
	ld2z R10
	jzU2 r11,L124; EQ 0
	jcI2 r11,r10,lbnf,L122; LT=lbnf i.e. subtract B from A and jump if borrow 
L130:
	ld2 R11,'O',sp,(-4+8) ;reg:INDIRI2(addr)
	jcI2I r11,23,lbnf,L122  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
	jnI2I r11,29,lbnf,L122; GT reverse  the subtraction
	shl2I R11,1
	ld2 R11,'O',R11,(L131-46);reg:  INDIRP2(addr)
	jumpv r11; JUMPV(reg)
L131:
	dw L126
	dw L128
...
;	if (*s1 != *s2)
	...
	jeqI2 r11,r10,L150; EQI2(reg,reg)
;	    return ((*(unsigned char *)s1 < *(unsigned char *)s2) ? -1 : +1);
	ld1 R11,'O',R12,0
	zExt R11 ;CVUI2: widen unsigned char to signed int (zero extend)
	ld1 R10,'O',R13,0
	zExt R10 ;CVUI2: widen unsigned char to signed int (zero extend)
	jcI2 r11,r10,lbdf,L153; GE is flipped test from LT
	ldaD R6,-1; reg:acon
	lbr L154
L153:
	ldaD R6,1; reg:acon
...
;	else if (*s1 == '\0')
	...
	jnzU1 r11,L155; NEI2(CVUI2(reg),con0)
;	    return 0;
	ld2z R15
	lbr L145
L155:
...
;	while (*str != 0) {
	ldn1 R11,R12;reg:  INDIRU1(indaddr)
	jnzU1 r11,L163; NEI2(CVUI2(reg),con0)
;   return slen;
...
L168:
	...
	jnzU1 r11,L167; NEI2(CVUI2(reg),con0)
;}
L166:
	release 4; release room for outgoing arguments
	popr r7
...
;   if (flt < 0.0) {
	ld4 RL10,'D',(L173),0;INDIRF4(addr)
	jcF4 RL6,RL10,lbdf,L171;GEF4(reg,reg) - reverse test
;      *output++ = '-' ;
...
L172:
;   if (dec_digits < 8) {
	jcI2I r1,8,lbdf,L177; GE is flipped test from LT
...
;	if (s<0){
	...
	jcI2I r11,0,lbdf,L182; GE is flipped test from LT
;		*bptr='-';bptr++;
...
L190:
	jcU2 r6,r7,lbdf,L187 ;LE is flipped test & operands
;		if (flag || r>0||k==1){
	jnzU2 r0,L194; NE 0
	jnzU2 r1,L194; NE 0
	jneU2I r7,1,L191; NE
...
;	if (s<0){
	...
	jcI4 RL6,RL10,lbdf,L196; GE is flipped test from LT
;		*bptr++='-';
...
;	if (x<10){
	ld1 R11,'O',sp,(0+6)
	zExt R11 ;CVUI2: widen unsigned char to signed int (zero extend)
	jcI2I r11,10,lbdf,L202; GE is flipped test from LT
;		putc(x+'0');
	ld1 R11,'O',sp,(0+6)
	zExt R11 ;CVUI2: widen unsigned char to signed int (zero extend)
...
;		if (c!='%'){
	cpy1 R11,R7
	zExt R11 ;CVUI2(reg)*: widen unsigned char to signed int (zero extend)*
	jeqU2I r11,37,L209;EQI2(reg,con)
;			putc(c);
	cpy1 R12,R7
	zExt R12 ;CVUI2(reg)*: widen unsigned char to signed int (zero extend)*
	Ccall _putc
...
;			switch (c){
	...
	jcI2I r0,99,lbnf,L230  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
	jnI2I r0,108,lbnf,L231; GT reverse  the subtraction
	cpy2 R11,R0
	shl2I R11,1
	ld2 R11,'O',R11,(L232-198);reg:  INDIRP2(addr)
	jumpv r11; JUMPV(reg)
L232:
	dw L216
	dw L214
	...
	dw L218
L230:
	jeqU2I r0,88,L217;EQI2(reg,con)
	lbr L211
L231:
	ldaD R11,115; reg:acon
	jeqI2 r0,r11,L215; EQI2(reg,reg)
	jcI2 r0,r11,lbnf,L211; LT=lbnf i.e. subtract B from A and jump if borrow 
L234:
	jeqU2I r0,120,L217;EQI2(reg,con)
	lbr L211
L214:
;					printint(*this++);
...
;    while(*ptr) {
	ld2 R11,'O',sp,(0+16);reg:  INDIRP2(addr)
	ldn1 R11,R11;reg:  INDIRU1(indaddr)
	jnzU1 r11,L206; NEI2(CVUI2(reg),con0)
;} //prtf
L205:
	release 4; release room for outgoing arguments
	popr r7
