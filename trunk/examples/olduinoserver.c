...
void sendstuff(){
	...
	strcpy((char *)buf,"HTTP/1.0 200 OK\r\nContent-Type: text/html\r\n\r\n"
						...
						"<p><form method=\"POST\">\r\n");
	sendrc=send0(buf,strlen((char *)buf)); 	// Now Send the HTTP Response

	// Create the HTTP Temperature and radio button Response
	strcpy((char *)buf,	"<p><strong><input type=\"radio\" name=\"rd\" value=\"0\" "
						"checked"
						">LED OFF\r\n"
						...
						"<input type=\"submit\">\r\n"
						"</form></span></body></html>\r\n");
	sendrc=send0(buf,strlen((char *)buf)); // Now Send the HTTP Remaining Response
}
...
void analyzeinput(){
	int postidx;
	ledmode=2; //no choice made of led mode
	printf("analyzing:\n%s\n",buf);
...
void main(void){
	...
	//printf("six=%d\n",strindex((char *)"GET / HTTP/1.1","GET /"));
	delay(1000);
