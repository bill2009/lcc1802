; generated by lcc-xr182b $Version: 5.0 - XR182b $ on Fri May 24 13:33:31 2013
...
;  printf("Reading RTR: %d\n",(SPI_Read(RTR)<<8)+SPI_Read(RTR+1));
	...
	st2 R11,'O',sp,(-22+64); ASGNI2(addr,reg)*;
...
;  printf("Reading GAR: %d.%d.%d.%d\n\n",SPI_Read(GAR + 0),SPI_Read(GAR + 1),
	...
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-24+64); ASGNI2(addr,reg)*;
	ldaD R12,2; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-26+64); ASGNI2(addr,reg)*;
	ldaD R12,3; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-28+64); ASGNI2(addr,reg)*;
	ldaD R12,4; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
...
;  printf("Reading SAR: %x:%x:%x:%x:%x:%x\n\n",SPI_Read(SAR + 0),SPI_Read(SAR + 1),
	...
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-30+64); ASGNI2(addr,reg)*;
	ldaD R12,10; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-32+64); ASGNI2(addr,reg)*;
	ldaD R12,11; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-34+64); ASGNI2(addr,reg)*;
	ldaD R12,12; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-36+64); ASGNI2(addr,reg)*;
	ldaD R12,13; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-38+64); ASGNI2(addr,reg)*;
	ldaD R12,14; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
...
;  printf("Reading SUBR: %d.%d.%d.%d\n\n",SPI_Read(SUBR + 0),SPI_Read(SUBR + 1),
	...
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-40+64); ASGNI2(addr,reg)*;
	ldaD R12,6; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-42+64); ASGNI2(addr,reg)*;
	ldaD R12,7; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-44+64); ASGNI2(addr,reg)*;
	ldaD R12,8; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
...
;  printf("Reading SIPR: %d.%d.%d.%d\n\n",SPI_Read(SIPR + 0),SPI_Read(SIPR + 1),
	...
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-46+64); ASGNI2(addr,reg)*;
	ldaD R12,16; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-48+64); ASGNI2(addr,reg)*;
	ldaD R12,17; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-50+64); ASGNI2(addr,reg)*;
	ldaD R12,18; reg:acon
	Ccall _SPI_Read; CALLI2(ar)
...
;	while (*str != 0) {
	...
	jnzU1 r11,L79; NEI2(CVUI2(reg),con0)
;   return slen;
...
L84:
	...
	jnzU1 r11,L83; NEI2(CVUI2(reg),con0)
;}
L82:
	release 4; release room for outgoing arguments
	popr r7
...
;   if (flt < 0.0) {
	ld4 RL10,'D',(L89),0;INDIRF4(addr)
	jcF4 RL6,RL10,lbdf,L87;GEF4(reg,reg) - reverse test
;      *output++ = '-' ;
...
L88:
;   if (dec_digits < 8) {
	jcI2I r1,8,lbdf,L93; GE is flipped test from LT
...
;	if (s<0){
	...
	jcI2I r11,0,lbdf,L98; GE is flipped test from LT
;		*bptr='-';bptr++;
...
L106:
	jcU2 r6,r7,lbdf,L103 ;LE is flipped test & operands
;		if (flag || r>0||k==1){
	jnzU2 r0,L110; NE 0
	jnzU2 r1,L110; NE 0
	jneU2I r7,1,L107; NE
...
;	if (s<0){
	...
	jcI4 RL6,RL10,lbdf,L112; GE is flipped test from LT
;		*bptr++='-';
...
;	if (x<10){
	ld1 R11,'O',sp,(0+6)
	zExt R11 ;CVUI2: widen unsigned char to signed int (zero extend)
	jcI2I r11,10,lbdf,L118; GE is flipped test from LT
;		putc(x+'0');
	ld1 R11,'O',sp,(0+6)
	zExt R11 ;CVUI2: widen unsigned char to signed int (zero extend)
...
;		if (c!='%'){
	cpy1 R11,R7
	zExt R11 ;CVUI2(reg)*: widen unsigned char to signed int (zero extend)*
	jeqU2I r11,37,L125;EQI2(reg,con)
;			putc(c);
	cpy1 R12,R7
	zExt R12 ;CVUI2(reg)*: widen unsigned char to signed int (zero extend)*
	Ccall _putc
...
;			switch (c){
	...
	jcI2I r0,99,lbnf,L146  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
	jnI2I r0,108,lbnf,L147; GT reverse  the subtraction
	cpy2 R11,R0
	shl2I R11,1
	ld2 R11,'O',R11,(L148-198);reg:  INDIRP2(addr)
	jumpv r11; JUMPV(reg)
L148:
	dw L132
	dw L130
	...
	dw L134
L146:
	jeqU2I r0,88,L133;EQI2(reg,con)
	lbr L127
L147:
	ldaD R11,115; reg:acon
	jeqI2 r0,r11,L131; EQI2(reg,reg)
	jcI2 r0,r11,lbnf,L127; LT=lbnf i.e. subtract B from A and jump if borrow 
L150:
	jeqU2I r0,120,L133;EQI2(reg,con)
	lbr L127
L130:
;					printint(*this++);
...
;    while(*ptr) {
	ld2 R11,'O',sp,(0+16);reg:  INDIRP2(addr)
	ldn1 R11,R11;reg:  INDIRU1(indaddr)
	jnzU1 r11,L122; NEI2(CVUI2(reg),con0)
;} //prtf
L121:
	release 4; release room for outgoing arguments
	popr r7
