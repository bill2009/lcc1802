; generated by lcc-xr182b $Version: 5.0 - XR182b $ on Wed Jun 19 13:04:53 2013
...
;    offaddr = (((ptr & 0x00FF) << 8 ) + SPI_Read(S0_RX_RD + 1));
	...
	Ccall _SPI_Read; CALLI2(ar)
	ld2 R10,'O',sp,(-6+22); reg:INDIRU2(addr)
...
;    if (buflen>hdrlen){
	jnU2I r6,16,lbdf,L51 ;LEU2 16 6 L51; LE is flipped test & subtraction
;		toread=hdrlen;
	ldaD R11,16; reg:acon
	st2 R11,'O',sp,(-4+22); ASGNU2(addr,reg)*
;		toskip=buflen-hdrlen-trlrlen;
	alu2I R11,R6,16,smi,smbi
	alu2I R11,R11,32,smi,smbi
...
;     if (timeout++ > 1000) {
	...
	jnU2I r11,1000,lbdf,L60 ;LEU2 1000 11 L60; LE is flipped test & subtraction
;       disconnect0();
	Ccall _disconnect0
;       return 0;
	ld2z R15
...
L58:
;    while (txsize < buflen) {
	jcU2 r1,r6,lbnf,L57; LT=lbnf i.e. subtract B from A and jump if borrow 
...
L89:
	...
	jnzU1 r11,L86; NEI2(CVUI2(reg),con0)
;  return -1;
	ldaD R15,-1; reg:acon
L85:
	release 6; release room for outgoing arguments
...
;	printf("input from %d.%d.%d.%d\n\n",SPI_Read(S0_DIPR + 0),SPI_Read(S0_DIPR + 1),SPI_Read(S0_DIPR + 2),SPI_Read(S0_DIPR + 3));
	...
	Ccall _SPI_Read; CALLI2(ar)
	cpy2 R11,R15 ;LOADI2(reg)*
	st2 R11,'O',sp,(-10+20); ASGNI2(addr,reg)*;
	...
	zExt R10 ;CVUI2(reg)*: widen unsigned char to signed int (zero extend)*
	st2 r10,'O',sp,(4); arg+f**
	ld2 R10,'O',sp,(-10+20) ;reg:INDIRI2(addr)
	zExt R10 ;CVUI2(reg)*: widen unsigned char to signed int (zero extend)*
	st2 r10,'O',sp,(6); arg+f**
	cpy2 R11,R15 ;LOADU2(reg)*
	zExt R11 ;CVUI2(reg)*: widen unsigned char to signed int (zero extend)*
...
;	else if (strncmp((char *)buf,"GET /",5)>=0)
	...
	st2 r11,'O',sp,(4); arg+f**
	Ccall _strncmp; CALLI2(ar)
	jcI2I r15,0,lbnf,L101  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
;		reqtype=get;
	ld2z R11
	st2 R11,'D',(_reqtype),0; ASGNI2(addr,reg)*;
L101:
...
;	  if (strindex((char *)buf,"rd=0") > 0)
	...
	jnI2I r15,0,lbdf,L107 ;LEI2 0 15 L107; LE is flipped test & subtraction
;		reqmode=0;
	ld2z R11
	st2 R11,'O',sp,(-4+20); ASGNI2(addr,reg)*;
	lbr L105
...
;	  else if (strindex((char *)buf,"rd=1") > 0)
	...
	Ccall _strindex; CALLI2(ar)
	jnI2I r15,0,lbdf,L105 ;LEI2 0 15 L105; LE is flipped test & subtraction
;		reqmode=1;
	ldaD R11,1; reg:acon
	st2 R11,'O',sp,(-4+20); ASGNI2(addr,reg)*;
;	}
...
L105:
;	if (reqmode==0){
	ld2 R11,'O',sp,(-4+20) ;reg:INDIRI2(addr)
	jnzU2 r11,L120; NE 0
...
;		if (reqmode==1){
	ld2 R11,'O',sp,(-4+20) ;reg:INDIRI2(addr)
	jneU2I r11,1,L123; NE
;			printf("setting led ON\n");
	ldaD R12,L125; reg:acon
	Ccall _printf
;			ledmode=1;
	ldaD R11,1; reg:acon
	st2 R11,'D',(_ledmode),0; ASGNI2(addr,reg)*;
...
;	switch (reqtype){
	ld2 R11,'D',(_reqtype),0 ;reg:INDIRI2(addr)
	jzU2 r11,L130; EQ 0
	jeqU2I r11,1,L130;EQI2(reg,con)
	jeqU2I r11,2,L132;EQI2(reg,con)
	lbr L128
L130:
;			printf("sending form \n");
	ldaD R12,L131; reg:acon
	Ccall _printf
;			sendstuff();
	Ccall _sendstuff
;			break;
...
;  	switch (socketstatus){
	...
	ld2z R10
	jzU2 r11,L143; EQ 0
	jcI2 r11,r10,lbnf,L141; LT=lbnf i.e. subtract B from A and jump if borrow 
L149:
	ld2 R11,'O',sp,(-4+8) ;reg:INDIRI2(addr)
	jcI2I r11,23,lbnf,L141  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
	jnI2I r11,29,lbnf,L141; GT reverse  the subtraction
	shl2I R11,1
	ld2 R11,'O',R11,(L150-46);reg:  INDIRP2(addr)
	jumpv r11; JUMPV(reg)
L150:
	dw L145
	dw L147
...
;	if (*s1 != *s2)
	...
	jeqI2 r11,r10,L169; EQI2(reg,reg)
;	    return ((*(unsigned char *)s1 < *(unsigned char *)s2) ? -1 : +1);
	ld1 R11,'O',R12,0
	zExt R11 ;CVUI2: widen unsigned char to signed int (zero extend)
	ld1 R10,'O',R13,0
	zExt R10 ;CVUI2: widen unsigned char to signed int (zero extend)
	jcI2 r11,r10,lbdf,L172; GE is flipped test from LT
	ldaD R6,-1; reg:acon
	lbr L173
L172:
	ldaD R6,1; reg:acon
...
;	else if (*s1 == '\0')
	...
	jnzU1 r11,L174; NEI2(CVUI2(reg),con0)
;	    return 0;
	ld2z R15
	lbr L164
L174:
...
;	while (*str != 0) {
	ldn1 R11,R12;reg:  INDIRU1(indaddr)
	jnzU1 r11,L182; NEI2(CVUI2(reg),con0)
;   return slen;
...
;   if (flt < 0.0) {
	ld4 RL10,'D',(L192),0;INDIRF4(addr)
	jcF4 RL6,RL10,lbdf,L190;GEF4(reg,reg) - reverse test
;      *output++ = '-' ;
...
L191:
;   if (dec_digits < 8) {
	jcI2I r1,8,lbdf,L196; GE is flipped test from LT
...
;	if (s<0){
	...
	jcI2I r11,0,lbdf,L201; GE is flipped test from LT
;		*bptr='-';bptr++;
...
L209:
	jcU2 r6,r7,lbdf,L206 ;LE is flipped test & operands
;		if (flag || r>0||k==1){
	jnzU2 r0,L213; NE 0
	jnzU2 r1,L213; NE 0
	jneU2I r7,1,L210; NE
...
;	if (s<0){
	...
	jcI4 RL6,RL10,lbdf,L215; GE is flipped test from LT
;		*bptr++='-';
...
;	if (x<10){
	ld1 R11,'O',sp,(0+6)
	zExt R11 ;CVUI2: widen unsigned char to signed int (zero extend)
	jcI2I r11,10,lbdf,L221; GE is flipped test from LT
;		putc(x+'0');
	ld1 R11,'O',sp,(0+6)
	zExt R11 ;CVUI2: widen unsigned char to signed int (zero extend)
...
;		if (c!='%'){
	cpy1 R11,R7
	zExt R11 ;CVUI2(reg)*: widen unsigned char to signed int (zero extend)*
	jeqU2I r11,37,L228;EQI2(reg,con)
;			putc(c);
	cpy1 R12,R7
	zExt R12 ;CVUI2(reg)*: widen unsigned char to signed int (zero extend)*
	Ccall _putc
...
;			switch (c){
	...
	jcI2I r0,99,lbnf,L249  ;LT=lbnf i.e. subtract immedB from A and jump if borrow
	jnI2I r0,108,lbnf,L250; GT reverse  the subtraction
	cpy2 R11,R0
	shl2I R11,1
	ld2 R11,'O',R11,(L251-198);reg:  INDIRP2(addr)
	jumpv r11; JUMPV(reg)
L251:
	dw L235
	dw L233
	...
	dw L237
L249:
	jeqU2I r0,88,L236;EQI2(reg,con)
	lbr L230
L250:
	ldaD R11,115; reg:acon
	jeqI2 r0,r11,L234; EQI2(reg,reg)
	jcI2 r0,r11,lbnf,L230; LT=lbnf i.e. subtract B from A and jump if borrow 
L253:
	jeqU2I r0,120,L236;EQI2(reg,con)
	lbr L230
L233:
;					printint(*this++);
...
;    while(*ptr) {
	ld2 R11,'O',sp,(0+16);reg:  INDIRP2(addr)
	ldn1 R11,R11;reg:  INDIRU1(indaddr)
	jnzU1 r11,L225; NEI2(CVUI2(reg),con0)
;} //prtf
L224:
	release 4; release room for outgoing arguments
	popr r7
