...
/*adapted by bill rowe 20-07-12 for testing thr lcc1802 compiler
* downloaded from http://www.camelforth.com/page.php?9
* changed object file suffix to .hex
* eliminated read of symbol listing
* changed output handling to just emit
* kill simulator on branch to self
* added option -v to report simulator progress and variables
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
boolean verbose = FALSE;		//allow some simulator outputs
long icount=0;				//count of instructions executed
...
void BRANCH (byte N) {		// 0x3N
	...
	switch (N) {
	case 0:
		//printf("R[P].b.lo(%d)==(ofs(%d)+1)\n",R[P].b.lo,ofs);
		if ((R[P].b.lo-2)==ofs){ //branch to self at end of main
			if (verbose) {
				printf("branch to self\n");
...
void LBRANCH (byte N) {		// 0xCN
	...
	case 0:	//LBR; 				break;	// LBR
		LBR;
		if (R[P].w==(PC-2)){ //long branch to self at end of main
			if (verbose) {
				printf("long branch to self\n");
				printf("%d instructions executed\n",icount);
			}
...
void CODE60 (byte N) {			// 0x6N
	...
	case 7: 			// OUT 7 - EMIT character
//		c = mread (PSP);	// lo byte
//		putchar (c & 0x7F);
//		PSP += 2;		// pop word
		putchar(mread(R[X].w) & 0x7f);
		R[X].w++;
		break;
...
int main (int argc, char *argv[]) {
	...
	return_stack = 0x7fff; //find_symbol ("RETURNST");
	param_stack  = 0x7fff; //find_symbol ("PARAMSTA");
	abort_addr   = 0x7fff; //find_symbol ("ABORT");
	end_dict     = 0x0; //find_symbol ("ENDDICT");

	if (verbose) {printf ("Starting address: %4.4X\n", R[0].w);}
	for (;;) {
